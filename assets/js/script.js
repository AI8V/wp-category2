...
(function () {
  ...
  const COURSE_DATA = {
    ...
    categories: {
      "Business": { count: 1, color: "success" },
      "Health": { count: 2, color: "info" },
        ...
        <div class="card h-100 shadow-sm course-card" 
          ...
          <div class="card-body">
            <div class="d-flex justify-content-between align-items-center mb-2">
              ...
              <p class="fs-5 fw-bold ${priceClass} mb-0">${priceDisplay}</p>
            </div>
            <h3 class="card-title">${course.title}</h3>
            <div class="text-muted d-flex card-meta gap-3 border-top pt-2 mt-2">
              <span><span class="bi bi-people-fill" aria-hidden="true"></span> ${course.students} Students</span>
              <span><span class="bi bi-book-fill" aria-hidden="true"></span> ${course.lessons} Lessons</span>
            </div>
          </div>
          <div class="course-card-overlay">
            <div class="d-flex align-items-center mb-2">
              <p class="mb-0 me-5">${course.level}</p>
              <p class="fs-5 fw-bold ${priceClass} mb-0 ms-5">${priceDisplay}</p>
            </div>
            <h3 class="card-title">${course.title}</h3>
            ...
            <a class="btn btn-light" role="button" href="#" onclick="viewCourse(${course.id})">View Detail</a>
          </div>
        </div>
      </div>
    `;
        ...
        if (pageItems.length === 0) {
          container.innerHTML = `
            <div class="col-12 text-center my-5">
              ...
              <p class="text-primary">Try adjusting your search or filter settings</p>
            </div>
          `;
        } else {
  ...
  window.viewCourse = (courseId) => {
    const course = ContentManager.getCourse(courseId);
    if (course) {
      alert(`Course: ${course.title}\nInstructor: ${course.instructor}\nDescription: ${course.description}`);
      console.log('Full course details:', course);
    }
  };
